...
| Legacy `exp1`/`exp2` placeholders are used for paper reproduction in DiscOS. | **Not implemented by design.** Synthetic `exp1`/`exp2` stubs were removed from `discos-core` to avoid implying paper fidelity; use EvidenceOS artifact pipeline for reviewer reproduction. | Removed | `crates/discos-core/src/experiments/mod.rs`, `docs/REPRODUCE_PAPER.md` |
