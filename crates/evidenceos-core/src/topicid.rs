...
const SEMANTIC_PHYS_HIR_ESCALATION_DISTANCE: u32 = 100;
...
pub fn compute_topic_id(metadata: &ClaimMetadata, signals: TopicSignals) -> TopicComputation {
    ...
    let mut material = Vec::with_capacity(DOMAIN_TOPIC_ID_V1.len() + 1 + payload.len());
    material.extend_from_slice(DOMAIN_TOPIC_ID_V1);
    material.push(0);
