...
Interoperability with the public EvidenceOS daemon is maintained by depending directly on the upstream `evidenceos-protocol` crate pinned to an immutable release tag, plus CI checks that ban local `evidenceos.*` protobuf definitions. DiscOS performs a fail-closed `GetServerInfo` handshake on connect (protocol major + proto hash) and only allows mismatches when `--allow-protocol-drift` is explicitly set. This policy avoids accidental drift in message fields, RPC names, or package conventions while keeping wire-level compatibility anchored to the canonical daemon contract. See `docs/PROTOCOL_VERSIONING.md` for the migration policy.
